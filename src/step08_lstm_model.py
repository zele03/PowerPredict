...
DEFAULT_HIDDEN_SIZE = 64  # Podrazumevana velicina skrivene memorije LSTM sloja
DEFAULT_NUM_LAYERS = 2  # Podrazumevani broj naslaganih LSTM slojeva
...
def build_lstm_config_name(config):
    """
    Kreira jedinstven naziv modela na osnovu hiperparametara.
    """
    return (
        f"lstm_seq{config['sequence_length']}"
...
def generate_lstm_grid_configs(search_space=LSTM_GRID_SEARCH_SPACE):
    """
    Generise sve validne kombinacije hiperparametara za LSTM grid search.
    """
    search_keys = [
        "sequence_length",
        "batch_size",
    ...
    for values in product(*(search_space[key] for key in search_keys)):
        ...
        config["name"] = build_lstm_config_name(config)
        configs.append(config)

    return configs


HYPERPARAMETER_CONFIGS = generate_lstm_grid_configs()


def import_torch():
    """
    Ucitava torch tek kada se pokrece LSTM trening.
    """
    try:
        import torch
...
def create_sequences(
    ...
    """
    Kreira LSTM sekvence koristeci istoriju iz context_df i targete iz eval_df.
    """
...
def build_lstm_model_class(nn, config):
    """
    Kreira LSTM model klasu nakon sto je torch dostupan.
    """
...
def train_lstm_model(model, train_loader, validation_loader, device, torch, nn, config):
    """
    Trenira LSTM model sa early stopping logikom.
    """
    loss_fn = nn.MSELoss()
...
def create_prediction_report(predictions_df, dataset_name, model_name=MODEL_NAME):
    """
    Kreira summary i hourly report za LSTM predikcije.
    """
...
def create_baseline_lstm_compare_report(
    baseline_report_df, lstm_report_df, model_name=MODEL_NAME
):
    """
    Kreira kompaktan uporedni report sa baseline i LSTM metrikama.
    """
    baseline_df = baseline_report_df.copy()
    lstm_df = lstm_report_df.copy()

    baseline_df["Model"] = BASELINE_MODEL_NAME
    baseline_df = baseline_df.drop(columns=["Baseline"], errors="ignore")

    for df in [baseline_df, lstm_df]:
        df["Hour"] = (
            df["Hour"]
            .fillna("")
            .astype(str)
    ...
    lstm_metrics = lstm_df[key_columns + metric_columns + ["Broj_redova"]].rename(
        columns={
            **{metric: f"LSTM_{metric}" for metric in metric_columns},
            "Broj_redova": "LSTM_Broj_redova",
        }
    )

    compare_df = baseline_metrics.merge(lstm_metrics, on=key_columns, how="inner")
    compare_df["Broj_redova"] = compare_df[
        ["Baseline_Broj_redova", "LSTM_Broj_redova"]
    ].min(axis=1)

    for metric in metric_columns:
        difference_column = f"{metric}_Difference_Baseline_minus_LSTM"
        improvement_column = f"{metric}_Improvement_percent"

        compare_df[difference_column] = (
            compare_df[f"Baseline_{metric}"] - compare_df[f"LSTM_{metric}"]
        ).round(4)
    ...
    compare_df["Better_Model_by_MAE"] = np.where(
        compare_df["MAE_Difference_Baseline_minus_LSTM"] > 0,
        model_name,
        BASELINE_MODEL_NAME,
    )
...
def create_best_lstm_summary(best_result):
    """
    Kreira kratak zapis o najboljoj LSTM konfiguraciji.
    """
...
def create_lstm_model_summaries(results):
    """
    Kreira tabelu sa hiperparametrima i test metrikama svih LSTM modela.
    """
    summary_rows = []
...
def save_best_lstm_outputs(best_result):
    """
    Cuva summary najboljeg LSTM modela i compare report sa baseline modelom.
    """
    lstm_logs_dir = Path("data/logs/lstm_logs")
    compare_logs_dir = Path("data/logs/compare_logs")
    lstm_logs_dir.mkdir(parents=True, exist_ok=True)
    compare_logs_dir.mkdir(parents=True, exist_ok=True)

    best_summary_df = create_best_lstm_summary(best_result)
    best_summary_df.to_csv(lstm_logs_dir / "best_lstm_model_summary.csv", index=False)
    ...
    best_lstm_test_report_df = best_result["report_df"][
        best_result["report_df"]["Dataset"] == "test"
    ].copy()

    compare_report_df = create_baseline_lstm_compare_report(
        baseline_test_report_df,
        best_lstm_test_report_df,
        best_result["config"]["name"],
    )
    compare_report_df.to_csv(
        compare_logs_dir / "baseline_lstm_compare_report.csv",
        index=False,
    )
...
def create_lstm_model(config):
    """
    Glavna funkcija za trening i evaluaciju LSTM modela.
    """
    ...
    LSTMRegressor = build_lstm_model_class(nn, config)
...
def create_lstm_models():
    """
    Trenira po jedan LSTM model za svaku konfiguraciju hiperparametara.
    """
    reports = []
    results = []

    for config in HYPERPARAMETER_CONFIGS:
        result = create_lstm_model(config)
    ...
    logs_dir = Path("data/logs/lstm_logs")
    logs_dir.mkdir(parents=True, exist_ok=True)
    combined_report_df.to_csv(logs_dir / "lstm_hyperparameter_report.csv", index=False)
    create_lstm_model_summaries(results).to_csv(
        logs_dir / "lstm_model_summaries.csv",
        index=False,
    )

    best_result = min(results, key=lambda result: result["validation_mae"])
    save_best_lstm_outputs(best_result)
